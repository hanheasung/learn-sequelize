...
const { sequelize } = require('./models'); //폴더내의 index.js 파일은 require할떄 이름 생략 ㄱㄴ

const app = express(); //Express 애플리케이션 생성
app.set('post',process.env.PORT || 3001); // 포트 설정
app.set('view engine', 'html'); //템플릿 엔진을 HTML로 설정


//nunjucks를 Express 에 설정
...
sequelize.sync({ force: false}) //force 옵션을 false로 설정하여 기존 데이터를 보존
    .then(() => {
        console.log('데이터 베이스 연결 성공'); //동기화 성공 -> 콘솔에 메시지 출력
    })
...
//라우터가 없는 경우 404 에러 발생시키는 미들웨어 추가
app.use((req,res,next) =>{
    // 요청된 HTTP 메소드와 URL에 대한 에러 메시지 생성
    const error = new Error(`${req.method} ${req.url} 라우터가 없습니다.`)
    error.status = 404; //상태코드 404(not found)로 설정
